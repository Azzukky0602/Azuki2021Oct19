...
if push == True:
    ...
    if bool(syusso) == False:
        ...
        for horse_id, df in past_results.items():
            ...
            df2 = df.drop(['天気', '映像', '頭数', '枠番', 'ﾀｲﾑ指数', '通過', 'ペース', '上り','騎手', 'R', '馬場指数', '斤量', 'オッズ', '人気', '馬体重', '厩舎ｺﾒﾝﾄ', '備考', '賞金', '勝ち馬(2着馬)', '日付', '距離', '馬番', '開催', '着順'], axis = 1)

            df3 = df2.loc[:, ['日付2', '開催2', 'レース名', '馬場', 'コース', '距離2', '着順2', '着差', 'タイム']].dropna()
            df4 = df3[(df3['着差'] < 3.5)]

            processed_horse_results[horse_id] = df4
